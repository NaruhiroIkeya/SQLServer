...
:: @summary:SQL Server System Database Full Backup
::
:: @since:2024/07/22
:: @version:1.0
:: @see:SQLSysDBFullBackup2S3.ps1
::
:: @param:none
...
SET __LOG_PREFIX__=SystemDBFullBackup
...
ECHO "%__SQLCMD__%" -E -S %__DBHOSTNAME__%\%__INSTANCENAME__% -i %__EXEC_SQL_FILE__% -o %__EXEC_LOG_FILE__% -v S3bucket = %__S3BUCKET__% -b -r1 >> "%__LOGFILE__%"

"%__SQLCMD__%" -E -S %__DBHOSTNAME__%\%__INSTANCENAME__% -i %__EXEC_SQL_FILE__% -o %__EXEC_LOG_FILE__% -v S3bucket = %__S3BUCKET__% -b -r1

IF ERRORLEVEL 1 (
  CALL :__ECHO__  Backup System Database Execute Error
...
DEL /Q %__EXEC_LOG_FILE__%
ECHO. >> "%__LOGFILE__%"
CALL :__ECHO__ ===== SQL Server Backup System Database Completed =====
ECHO. >> "%__LOGFILE__%"
